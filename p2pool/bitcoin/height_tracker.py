...
@defer.inlineCallbacks
def get_height_rel_highest_func(dimecoind, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(dimecoind.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield dimecoind.rpc_getblock('%x' % (block_hash,))
