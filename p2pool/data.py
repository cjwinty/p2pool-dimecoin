...
def get_warnings(tracker, best_share, net, dimecoind_warning, dimecoind_work_value):
    res = []
    ...
    if dimecoind_warning is not None:
        if 'This is a pre-release test build' not in dimecoind_warning:
            res.append('(from dimecoind) %s' % (dimecoind_warning,))
    
    if time.time() > dimecoind_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH dimecoind for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - dimecoind_work_value['last_update']),))
